  ...
  def update_meb(self, pt_cp, cam_cp, alt_cp, ext_cp) -> tuple[structs.CarState, structs.CarStateSP, structs.CarStateIC]:
    ...
    if self.CP.flags & VolkswagenFlags.ALT_GEAR:
      ret.gearShifter = self.parse_gear_shifter(
        self.CCP.shifter_values.get(pt_cp.vl["Gateway_73"]["GE_Fahrstufe"], None))  # candidate for MEB and MQB Evo
    else:
    ...
    ret.stockFcw = (bool(ext_cp.vl["AWV_03"]["FCW_Active"])
                    if not (self.CP.flags & VolkswagenFlags.DISABLE_RADAR) else False)  # most plausible candidate

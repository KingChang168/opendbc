...
class LongControlLimit:
  ...
  def update(self, enabled: bool, speed: float, set_speed: float, distance: float, has_lead: bool, critical_state: bool):
    ...
    if not enabled or critical_state: # force most precise accel command execution
      ...
      # For new lead detection, reset the filter and force the current state on the next iteration.
      ...
      upper_limit_dist_change = np.interp(distance, self.LIMIT_DISTANCE_CHANGE_UP_ACT,
                                          [upper_limit_dist_change, upper_limit_dist])  # distance change activation
      ...
      # Set speed reduction requested by the user or speed overshoot, including HUD-to-real-speed difference.

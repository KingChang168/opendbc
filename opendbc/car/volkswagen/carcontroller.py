...
class CarController(CarControllerBase, IntelligentCruiseButtonManagementInterface):
  def __init__(self, dbc_names, CP, CP_SP, CP_IC):
    ...
    self.accel_last = 0.
    is_meb_or_mqb_evo = bool(self.CP.flags & (VolkswagenFlags.MEB | VolkswagenFlags.MQB_EVO))
    self.long_jerk_control = LongControlJerk(dt=(DT_CTRL * self.CCP.ACC_CONTROL_STEP)) if is_meb_or_mqb_evo else None
    self.long_limit_control = LongControlLimit(dt=(DT_CTRL * self.CCP.ACC_CONTROL_STEP)) if is_meb_or_mqb_evo else None
  ...
  def update(self, CC, CC_SP, CC_IC, CS, now_nanos):
    ...
    # "Wechselblinken" switches between hazards and one-sided indicators for each indicator cycle
    ...
    if self.frame % self.CCP.ACC_CONTROL_STEP == 0 and self.CP.openpilotLongitudinalControl and not CS.out_ic.radarDisableFailed:
      ...
      if self.CP.flags & (VolkswagenFlags.MEB | VolkswagenFlags.MQB_EVO):
        ...
        #     for a clean exit of ACC car controls
      ...
      if self.CP.flags & (VolkswagenFlags.MEB | VolkswagenFlags.MQB_EVO):
        ...
        if self.frame % self.CCP.AEB_HUD_STEP == 0:
          can_sends.append(self.CCS.create_aeb_hud(self.packer_pt, self.CAN.pt,
                                                   self.radar_disabled_warning_timer < 600))  # AEB HUD (5 Hz)
